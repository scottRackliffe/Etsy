...
> converged — verified), WP4 ✅ backend, WP6 ✅, WP7 ✅, C16 ✅. **REMAINING = BUILDS/OPS ONLY (no
> docs):** (a) WP5 build — design done, needs owner green-light + `ai.premium_model`; (b) WP4 UI
> (dashboard tax badge + 3 Settings inputs); (c) WP3 full bootstrap *deletion* — gated on a
> `npm run dev` boot smoke-test; (d) ops: run `npm run db:migrate` on the live DB. The WP5 `model?`
> code hook is held in `git stash` for reuse.
>
...
| **WP5 — 3-phase model engagement (ADR-086 §1a + ADR-089)** | Score/lifecycle/remediation-driven tiered escalation; replaces static two-tier. **Design now split: tiers/cost → ADR-086 §1a; flow + user-observed cycle (Stop/Cycle/Advance) → ADR-089.** Prepped `model?` hook held in git stash. | D1 | code+test | Medium (larger) | ⏸ NOT BUILT — **blocked on owner-supplied facts, deliberately not guessed.** Design documented (ADR-086 §1a). Implementation needs: (1) the tier→model assignments + score-band thresholds (which model at which score), and (2) **live AI verification** (real generate→evaluate runs measured vs the 85 gate) — neither can be invented or done headless without the owner's model choices + API spend. Build once those facts are provided. |
