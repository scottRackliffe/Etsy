...
| GET    | /api/inventory/[id]/listing-readiness        | App  | Listing phase + context-aware button + data remediation (ADR-081) | Path: id                                                                                   | 200: `{ item_id, ready, listing_phase, button: { label, action }, required: [{field, present}], data_remediation: [{field, label, present, required, shortcoming, resolution_link}], missing_fields, checks, picture_count }`. Recomputes + persists `listing_phase`. `button.action` ∈ `evaluate_data` \| `generate` \| `evaluate_quality`. Same preflight rules: item_number, description, condition_code, sale_revenue (>0), ≥1 picture. 404 if not found.                                                                                                    |
...
> AP Lite chip; `entity_type=customer,address` for Customers). Single-value usage is unchanged.
