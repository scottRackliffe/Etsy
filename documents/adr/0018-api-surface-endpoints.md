...
| GET    | /api/inventory/[id]/listing-readiness        | App  | Listing phase + context-aware button + data remediation (ADR-081) | Path: id                                                                                   | 200: `{ item_id, ready, listing_phase, button: { label, action }, required: [{field, present}], data_remediation: [{field, label, present, required, shortcoming, resolution_link}], missing_fields, checks, picture_count }`. Recomputes + persists `listing_phase`. `button.action` ∈ `evaluate_data` \| `generate` \| `evaluate_quality`. Preflight rules (ADR-085: price removed): item_number, description, condition_code, ≥1 picture. 404 if not found.                                                                                                    |
| ~~POST~~   | ~~/api/inventory/[id]/listing-export~~           | —  | **REMOVED (ADR-085):** portable export retired.       | —                                                                                               | —                                                                                                                                                                                                                                                                             |
| ~~POST~~   | ~~/api/inventory/[id]/listing-import~~           | —  | **REMOVED (ADR-085):** portable import retired.       | —                                                                           | —                                                                                                                                                                                                                                                        |
| ~~POST~~   | ~~/api/inventory/[id]/listing-approve~~          | —  | **REMOVED (ADR-085):** approve step retired; publish gate is `listing_phase = 'listing_ready'`. | —                                                                                               | —                                                                                                                                                                                                                                                                                    |
| POST   | /api/inventory/[id]/publish-to-etsy          | App  | Publish listing to Etsy (at `listing_ready`)          | Path: id; Body: `{ mode?: "create" \| "update" }`                                                      | Requires `listing_phase = 'listing_ready'` (ADR-085 §5) + Etsy publish settings. Per-item fields (`etsy_when_made`, `etsy_taxonomy_id`, `etsy_who_made`, `etsy_shipping_profile_id`, `etsy_return_policy_id`) override global defaults per ADR-017 §1c. Sends `who_made`, `when_made`, `taxonomy_id`, `shipping_profile_id`, `return_policy_id`, `readiness_state_id`, `materials[]`, `item_weight/length/width/height` + units, `is_supply`, `type=physical`. Uploads local pictures (1–20) + optional video one-by-one with retry. **First publish** (no `etsy_listing_id`, or `mode:"create"`): `createDraftListing` → activate → persists **new** `etsy_listing_id`, sets `is_listed=1`, `status='Listed'`, `listing_published_at`. **Re-publish guard (ADR-085 §5):** if the item already has `etsy_listing_id` and no `mode` is supplied → **409 `ALREADY_PUBLISHED`** `{ etsy_listing_id, actions: ["update","create"] }` (client must confirm). `mode:"update"` → Etsy `updateListing(etsy_listing_id, …)` (no duplicate; refresh fields/images; bump `listing_published_at`). `mode:"create"` → new listing (duplicate; previous untouched). Logs `listing.published` with `detail_json.mode`. Blocks if any required Etsy field missing (400), if not `listing_ready` (409 `PUBLISH_NOT_READY`), or if no images available (409). |
...
| POST   | /api/inventory/[id]/generate-listing-content | App  | Generate full listing via AI (research + price + all fields) | Path: id                                                                                          | Preflight (ADR-085 §2, price removed): item_number, description, condition_code, ≥1 picture. Runs the full Generate engine (ADR-085 §3): web-search **price recommendation** (writes recommended `sale_revenue`, editable), identification, and **all** listing fields (title/description/tags/category_path + strategy fields), plus suggested `etsy_when_made`/`etsy_taxonomy_id`/materials/dimensions/`picture_classifications`, evidence/citations/compliance. Sends **all** non-empty pictures + item context. On success records `listing_generated_at` + `listing_source_hash` and moves item to phase `generated` (ADR-081). 200: `{ listing_title, listing_description, listing_tags, listing_category_path, price, evidence, citations, compliance_check, listing_phase }`. 400 with field errors if prerequisites missing. 404 if not found. |
...
| POST   | /api/inventory/[id]/annotate-dimensions      | App  | Render measurement photo (ADR-084)                   | JSON: `{ length?, width?, height?, unit, target_slot?, write_back? }`                                  | Renders an annotated copy of `picture_1` with dual-unit callouts (Sharp), saves it to a **secondary** slot (2–20, never the hero) classified `measurement`, optionally writes the confirmed values back to `item_length/width/height` + `item_dimensions_unit`. Persists `dimension_annotation_json`. Returns `{ ok, slot, picture_path, alt_text, item }`. Logs `inventory.dimensions_annotated`. 400 if no dimensions / invalid slot; 409 if no empty slot; 404 if item not found. |
...
Query: `entity_type?`, `entity_id?`, `action?`, `source?`, `from_date?`, `to_date?`,
`limit?` (default 50, max 200), `offset?`.

> **WS-A addendum (2026-06-21, ADR-037 §A5):** `entity_type` accepts a **comma-separated list**
> so one filter chip can span multiple types (e.g. `entity_type=expense,tax_payment` for the
> Expenses chip; `entity_type=customer,address` for Customers). Single-value usage is unchanged.
> The **System** chip is a composite: it matches `entity_type IN ('system')` **OR**
> `action LIKE 'auth.%'`; the **Config** chip matches `entity_type='setting'` **excluding**
> `action LIKE 'auth.%'`. These composites are resolved server-side. Canonical `entity_type`
> values: `inventory, order, customer, address, receipt, vendor, expense, tax_payment, shipping,
> report, setting, sync, backup, system` (ADR-037 §A1). The Recent Activity dashboard view calls
> this with `limit=25` and no filters (ADR-016 §6).
...
~~`GET /api/inventory/[id]/listing-score`~~ → **REMOVED (ADR-085):** the ADR-068 score is retired. Use the deterministic fast path of the ADR-082 rubric (via `listing-quality` / cached `listing_quality_json`) instead.
...
**B29) Listing Coach (§29, ADR-072) — REMOVED (ADR-085, 2026-06-21)**

> **Superseded:** the `/api/listing-coach/*` routes below are removed. The request/response shapes
> are retained here for historical reference only; the same multipart photo intake + research/
> compose/refine logic now lives behind `POST /api/inventory/[id]/generate-listing-content`,
> `/listing-refine`, and `/listing-video`, and item creation is `POST /api/inventory`. See ADR-085.
...
| Method | Path          | Auth | Purpose              | Request                                                                | Response / behavior                                                                                                                                         |
| ------ | ------------- | ---- | -------------------- | ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | /api/activity | App  | Activity log entries | Query: entity_type? (comma-separated, WS-A), entity_id?, action?, source?, from_date?, to_date?, limit? (default 50, max 200), offset? | 200: `{ items: [ { id, action, entity_type, entity_id, entity_label, detail_json, source, created_at } ], pagination }`. `source`: user, system, etsy_sync. entity_type values per ADR-037 §A1. Composite chips (System/Config) resolved server-side (ADR-037 §A5). |
...
| ~~GET~~    | ~~/api/inventory/[id]/listing-score~~ | —  | **REMOVED (ADR-085):** ADR-068 score retired; use ADR-082 rubric |
...
**§29. Listing Coach (ADR-072) — REMOVED (ADR-085, 2026-06-21)**

The standalone `/api/listing-coach/*` routes are **removed**. Their capabilities moved into the
inventory lifecycle: research + price + full-field authoring → `POST
/api/inventory/[id]/generate-listing-content`; refine → `POST /api/inventory/[id]/listing-refine`;
video → `POST /api/inventory/[id]/listing-video`. New items are created via `POST /api/inventory`
(inline SEMS create). See ADR-085.

| Method | Path                          | Auth | Status                                                            |
| ------ | ----------------------------- | ---- | ------------------------------------------------------------------ |
| ~~POST~~   | ~~`/api/listing-coach/analyze`~~  | —  | **REMOVED** → `generate-listing-content`                       |
| ~~POST~~   | ~~`/api/listing-coach/compose`~~  | —  | **REMOVED**                                                    |
| ~~POST~~   | ~~`/api/listing-coach/complete`~~ | —  | **REMOVED** → `POST /api/inventory` (inline create)           |
| ~~POST~~   | ~~`/api/listing-coach/refine`~~   | —  | **REMOVED** → `listing-refine`                                |
...
**§39. Communications & customer outreach (ADR-078)** — added 2026-06-21

Payment reminders and thank-you notes. **Compliance enforced server-side:** `payment_reminder`
sends are rejected (400) for any order with `source_channel != 'manual'`.

| Method | Path | Auth | Purpose | Request / response |
| ------ | ---- | ---- | ------- | ------------------ |
| GET  | `/api/communications/candidates` | App | Eligible orders for a message type | Query: `type` (`payment_reminder`\|`thank_you`), pagination. 200: `{ items:[{ order_id, order_number, customer_id, customer_name, customer_email, grand_total, order_date, is_shipped, already_reminded_at? }], pagination }` (queries per ADR-078 §3). |
| POST | `/api/communications/preview` | App | Render merged message (no send) | Body `{ type, order_id }`. 200: `{ subject, body, channel_default, unknown_tokens:[] }`. |
| POST | `/api/communications/send` | App | Send/print a batch | Body `{ type, channel:("email"\|"print"), order_ids:[] }`. 200: `{ results:[{ order_id, status, error? }] }`. Writes `communication_log`; logs `communication.sent`. 400 if `payment_reminder` + non-manual order, or email channel with `email.enabled!="true"`. |
| GET  | `/api/communications/log` | App | Outreach history | Query: `type?`, `order_id?`, pagination. 200: `{ items, pagination }`. |
| GET  | `/api/reports/payment-reminder/[orderId]` | App | Per-order payment-reminder PDF letter (ADR-036 pattern) | Query: `format=pdf`. 404 if order not found. |
| PUT  | `/api/settings/email` | App | Save SMTP settings | Body SMTP fields; password encrypted at rest (ADR-025). 200: masked summary. |
| POST | `/api/settings/email/test-connection` | App | Verify SMTP login | 200: `{ ok, message }`. |

Full spec: ADR-078. `communication_log` schema: ADR-017 §6h. Email settings: ADR-034.
