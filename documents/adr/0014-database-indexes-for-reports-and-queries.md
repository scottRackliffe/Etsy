...
**Why index?** Indexes let the database find rows quickly by column value (e.g. “all orders in this date range”, “all orders for this customer”, “group by shipper”). We define indexes as part of the schema so report and filter queries use them from day one.
...
- **Orders table:** index on **order_date** (for date-range reports, MTD/YTD, sales by period); index on **customer_id** (for “orders by customer”, thank-you and invoice lookups); index on **shipper** if we filter or group by it (e.g. postal-by-vendor report).
...
> **Reconciled 2026-06-09:** Terminology updated to match ADR-017 schema. "Purchase/sales table" → "Orders table"; "date of purchase" → "order_date"; "orders by customer" → "orders by customer".
