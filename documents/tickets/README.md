...
| ~~WS-CR6~~ | [WS-CR6_generate-concurrency-baseline.md](WS-CR6_generate-concurrency-baseline.md) | "Record modified" / `PATCH 409` concurrency on save — **CONFIRMED** (live log) | ✅ DONE (code) |
| ~~WS-CR7~~ | [WS-CR7_reasoning-model-tier-support.md](WS-CR7_reasoning-model-tier-support.md) | Reasoning-class model tier support (drop temperature + reasoning-effort dial) — live-test finding | ✅ DONE (code) |
...
| ~~WS-CR10~~ | [WS-CR10_surface-real-ai-errors.md](WS-CR10_surface-real-ai-errors.md) | Surface the real error in AI dialogs (stop masking causes; don't discard billed AI on save-fail) — live-test finding | ✅ DONE (code) |
| ~~WS-CR11~~ | [WS-CR11_error-dialog-details-button.md](WS-CR11_error-dialog-details-button.md) | Error dialogs: add a "Details" button revealing code/message/timestamp (+ Copy) — owner request | ✅ DONE (code) |
| ~~WS-CR12~~ | [WS-CR12_generate-timeout-many-images.md](WS-CR12_generate-timeout-many-images.md) | Generate times out (~30s) with many images; server saves but UI false-fails — live-test finding | ✅ DONE (code) |
