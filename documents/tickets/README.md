...
| **WS-CR15** | [WS-CR15_per-row-fix-ai-fixable.md](WS-CR15_per-row-fix-ai-fixable.md) | Per-row "Fix" button on AI-fixable remediation rows (reuse listing-refine) — live-test finding | Tier 2 (UX) → Sonnet |
| **WS-CR16** | [WS-CR16_rubric-honors-publish-defaults.md](WS-CR16_rubric-honors-publish-defaults.md) | Rubric must honor publish defaults (who_made/when_made) — stops false nag (C7 sibling) | Tier 2 → Sonnet |
| **WS-CR17** | [WS-CR17_pending-photo-row-copy-and-ux.md](WS-CR17_pending-photo-row-copy-and-ux.md) | "Per-photo AI pending" row: stale "WS-G3" copy + misleading Fix link — live-test finding | Tier 2 (UX) → Sonnet |
| **WS-CR18** | [WS-CR18_photo-vision-empty-economy-model.md](WS-CR18_photo-vision-empty-economy-model.md) | Per-photo AI vision returns empty → score stuck provisional — **diagnose first** | **Tier 1 — owner-led** |
