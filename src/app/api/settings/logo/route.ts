...
const RELATIVE_PATH = "uploads/branding/logo.png";

function getAbsolutePath(): string {
  const root = process.env.UPLOADS_PATH || path.join(process.cwd(), "uploads");
  return path.join(root, "branding", "logo.png");
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    setSetting("business_logo_path", RELATIVE_PATH);
    ...
    return errorResponse(
      fromUnknownError(error, {
        ...
        userMessage: "We could not save the business logo.",
...
export async function DELETE() {
  try {
    ...
    const current = getSetting("business_logo_path");
    if (current) {
    ...
    return errorResponse(
      fromUnknownError(error, {
        ...
        userMessage: "We could not remove the business logo.",
