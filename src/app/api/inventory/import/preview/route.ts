...
import { MAX_FILE_BYTES, previewInventoryCsv } from "@/lib/inventory-csv-import";
  ...
  try {
    ...
    if (!(file instanceof File)) {
      throw new ApiRouteError({
        ...
        userMessage: "Choose a CSV file to preview.",
    ...
    const result = previewInventoryCsv(buffer);
    if ("error" in result) {
      throw new ApiRouteError({
        status: 400,
        code: "VALIDATION_ERROR",
    ...
    return errorResponse(
      fromUnknownError(error, {
        ...
        userMessage: "We could not preview the CSV file.",
