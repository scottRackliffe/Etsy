...
import { batchInventory, type BatchResult } from "@/lib/batch-operations";
import { parseBatchIdList, type BatchRequestBody } from "@/lib/batch-request";

const VALID = new Set(["change_status", "delete"]);
...
export async function POST(request: NextRequest) {
  ...
  if (!VALID.has(action)) {
    return errorResponse(
      new ApiRouteError({
        ...
        actions: ["Use change_status or delete."],
        canRetry: false,
      })
    );
  }
  let idList: number[];
  try {
    idList = parseBatchIdList("inventory", body);
  } catch (err) {
  ...
  try {
    ...
    for (let i = 0; i < idList.length; i += CHUNK_SIZE) {
      ...
      const result = batchInventory(action, chunk, body.params ?? {});
